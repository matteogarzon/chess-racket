...
(define (player-connection listener black-color white-color)
  (displayln "Waiting for players to connect...")
  
  ; Accept first player connection
  (define-values (first-in first-out first-connection)
    (tcp-accept listener))
  (displayln "First player connected")
  (write black-color first-out)
  (flush-output first-out)
  
  ; Accept second player connection
  (define-values (second-in second-out second-connection)
    (tcp-accept listener))
  (displayln "Second player connected")
  (write white-color second-out)
  (flush-output second-out)
  
  ; Return both connections
  (values 
   (make-connection first-in first-out)
   (make-connection second-in second-out)))
